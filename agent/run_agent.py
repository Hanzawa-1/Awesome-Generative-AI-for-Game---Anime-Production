...
def daily_sweep_tasks(tax, day_index: int, num_days: int = 7) -> list[str]:
    """Today's slice of the full task list for the rolling daily sweep.

    Splits ALL task ids (in ``tax.all_pairs()`` stable sorted order) into ``num_days``
    contiguous, as-even-as-possible buckets and returns the bucket for ``day_index``
    (0-based; Mon=0 .. Sun=6 in the workflow). The split is derived from the LIVE
    taxonomy, so the per-day size adapts automatically as tasks are added or removed
    (53 tasks -> 8/8/8/8/7/7/7; 64 -> 10/9/9/9/9/9/9). Over one full cycle of ``num_days``
    runs, every task is covered exactly once with no overlap."""
    task_ids = [t for _, t in tax.all_pairs()]
    n = len(task_ids)
    if n == 0 or num_days <= 0:
        return []
    day_index %= num_days
    base, extra = divmod(n, num_days)          # the first `extra` buckets get one more
    start = day_index * base + min(day_index, extra)
    size = base + (1 if day_index < extra else 0)
    return task_ids[start:start + size]
