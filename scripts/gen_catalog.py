...
def _compute_new_ids(entries) -> set[str]:
    """Flag entries from the most recent merge batch (those sharing the newest date_added).
    The badge stays on that batch until a later-dated PR lands a newer one — i.e. "new until
    the next PR", typically about a week.

    Suppressed (returns empty) when that batch IS the whole catalog or a clear majority of it
    — the seed + backfill bootstrap all share one date, and we must not blanket every card.
    Once any older-dated entries exist, a normal PR-sized batch is badged.
    ...
    cohort = {e.id for e in entries if e.date_added == latest}
    has_older = any(e.date_added < latest for e in entries)
    if not has_older or len(cohort) > len(entries) * 0.5:
        return set()  # bootstrap / bulk op, not a PR batch — don't blanket the catalog
    return cohort


def _write_nav_counts(entries, new_ids: set[str]) -> None:
    """Emit docs/javascripts/nav-counts.js mapping catalog page paths -> count of NEW entries,
    so nav-badges.js can pin a notification-style number on each area/task in the left nav
    (and the Catalog root). Always written — even ``{}`` — so the extra_javascript reference
    never 404s under ``mkdocs build --strict``."""
    new = [e for e in entries if e.id in new_ids]
    per_area = Counter(e.area for e in new)
    per_task = Counter((e.area, e.task) for e in new)
    counts: dict[str, int] = {}
    if new:
        counts["catalog/index.html"] = len(new)
    for area_id, n in per_area.items():
        counts[f"catalog/{area_id}/index.html"] = n
    for (area_id, task_id), n in per_task.items():
        counts[f"catalog/{area_id}/{task_id}.html"] = n
    js = "window.NAV_NEW_COUNTS = " + json.dumps(counts, sort_keys=True) + ";\n"
    _write("javascripts/nav-counts.js", js)
...
def main() -> int:
    ...
    _write_nav_counts(entries, new_ids)
